...
def make_request_form(method, endpoint, data=None):
    ...
    try:
        ...
        if method.upper() == "POST":
            ...
            return None, f"Unsupported method for form data: {method}"
...
user_data = {
    ...
    "full_name": "Free Access Test User",
    "plan": "starter",
    "industry": "Construction"
}

# Use form data for this endpoint
response, error = make_request_form("POST", "/admin/create-test-user", user_data)
if error:
    ...
    if response.status_code == 200:
        try:
            ...
            if result.get("status") == "success":
                ...
                test_result("Create Test User", True, f"Test user created: {user_id}")
            else:
                test_result("Create Test User", False, f"User creation failed: {result}")
        except json.JSONDecodeError:
            test_result("Create Test User", False, "Invalid JSON response")
    else:
        test_result("Create Test User", False, f"HTTP {response.status_code}: {response.text}")

# Test 7: Redeem Valid Professional Code
if generated_codes and created_users:
    print("7️⃣ Testing Valid Code Redemption...")
    professional_code = generated_codes[0]  # Use first generated code
    test_user_id = created_users[0]
...
if len(generated_codes) >= 2:
    ...
    if response and response.status_code == 200:
        try:
            ...
            if error:
                ...
                if response.status_code == 200:
                    ...
                    if response and response.status_code == 200:
                        try:
                            result = response.json()
                            third_user_id = result["user"]["id"]
                            created_users.append(third_user_id)
                            ...
                            test_result("Code Usage Limit Test", False, "Failed to parse third user response")
                    else:
            ...
            test_result("Code Usage Limit Test", False, "Failed to parse second user response")
    else:
...
if generated_codes and len(created_users) >= 2:
    ...
    if response and response.status_code == 200:
        try:
            ...
            test_result("Deactivated Code Rejection", False, "Failed to parse fourth user response")
    else:
...
if created_users:
    ...
    for i, user_id in enumerate(created_users, 1):
        print(f"   {i}. {user_id}")
    print()
